...
module.exports = class NotificationsDBApi {
  static async create(data, options) {
    ...
    await notifications.setFamily(data.family || null, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.family = await notifications.getFamily({
      transaction,
    });
  ...
  static async findAll(filter, globalAccess, options) {
    ...
    if (filter) {
      ...
      if (filter.content) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('notifications', 'content', filter.content),
        };
      }
  ...
  static async findAllAutocomplete(query, limit, globalAccess, organizationId) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('notifications', 'content', query),
        ],
      };
    }
