      ...
      <SectionMain>
        ...
        <CardBox>
          <div className={'mb-4'}>
            ...
            <p>{chats?.user?.firstName ?? 'No data'}</p>
          </div>

          <FormField label='Multi Text' hasTextareaHeight>
            <textarea className={'w-full'} disabled value={chats?.message} />
          </FormField>
            ...
            {chats.timestamp ? (
              <DatePicker
                ...
                selected={
                  chats.timestamp
                    ? new Date(
                        dayjs(chats.timestamp).format('YYYY-MM-DD hh:mm'),
                      )
                    : null
                }
                disabled
